-- 🎯 LEONLOGIC CMS SAFE SETUP (NO DESTRUCTIVE OPERATIONS)
-- Run this script in your Supabase SQL Editor
-- This will NOT affect your existing audit_leads table
...
-- Create policies (without dropping existing ones)
CREATE POLICY IF NOT EXISTS "Admin users can view their own data" ON admin_users
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Only admins can manage users" ON admin_users
  FOR ALL USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Public can view active hero content" ON hero_content
  FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Public can view active services" ON services
  FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Public can view published blog posts" ON blog_posts
  FOR SELECT USING (status = 'published' AND is_active = true);

CREATE POLICY IF NOT EXISTS "Public can view active testimonials" ON testimonials
  FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Public can view active FAQs" ON faqs
  FOR SELECT USING (is_active = true);

CREATE POLICY IF NOT EXISTS "Public can view company info" ON company_info
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Public can view active page sections" ON page_sections
  FOR SELECT USING (is_active = true);

-- Admin policies - allow authenticated users with appropriate roles to manage content
CREATE POLICY IF NOT EXISTS "Admins and editors can manage hero content" ON hero_content
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
...
CREATE POLICY IF NOT EXISTS "Admins and editors can manage services" ON services
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() AND role IN ('admin', 'editor') AND is_active = true
...
CREATE POLICY IF NOT EXISTS "Admins and editors can manage blog posts" ON blog_posts
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() AND role IN ('admin', 'editor') AND is_active = true
...
CREATE POLICY IF NOT EXISTS "Admins and editors can manage testimonials" ON testimonials
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() AND role IN ('admin', 'editor') AND is_active = true
...
CREATE POLICY IF NOT EXISTS "Admins and editors can manage FAQs" ON faqs
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() AND role IN ('admin', 'editor') AND is_active = true
...
CREATE POLICY IF NOT EXISTS "Only admins can manage company info" ON company_info
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
...
CREATE POLICY IF NOT EXISTS "Admins and editors can manage page sections" ON page_sections
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM admin_users 
      WHERE user_id = auth.uid() AND role IN ('admin', 'editor') AND is_active = true
...
-- Create triggers for updated_at (without dropping existing ones)
...
-- Show existing audit_leads table (should remain untouched)
SELECT 'audit_leads table exists and is untouched' as audit_status;
...
-- 
-- Your existing audit_leads table remains untouched! 